/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parse.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: javiersa <javiersa@student.42malaga.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/10 11:50:47 by javiersa          #+#    #+#             */
/*   Updated: 2023/05/10 12:30:21 by javiersa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

void	check_args(int narg, char **argv)
{
	int	i;

	i = 0;
	if (narg != 5 && narg != 6)
		ft_error(INSTRUCTIONS);
	while (--narg > 0)
	{
		i = -1;
		while (argv[narg][++i])
			if (argv[narg][i] < '0' || argv[narg][i] > '9')
				ft_error("Arguments can only be unsigned \
				positive integers.");
	}
}

unsigned int	ft_atoui(const char *str)
{
	unsigned long	number;
	int				i;

	number = 0;
	i = 0;
	while ((str[i] >= 9 && str[i] <= 13) || str[i] == 32)
		i++;
	if (str[i] == '+')
		i++;
	else if (str[i] == '-')
		ft_error("Only positive values ​​are accepted.");
	while (str[i] >= '0' && str[i] <= '9')
	{
		number = (number * 10) + (str[i] - '0');
		i++;
	}
	if (number > 4294967295)
		ft_error("Values ​​must be in the range of unsigned int.");
	return (number);
}

void	parse_data(int narg, char **argv, t_data *data)
{
	data->ends = 0;
	data->stop = 1;
	data->n_philos = ft_atoui(argv[1]);
	data->time_dead = ft_atoui(argv[2]);
	data->time_eat = ft_atoui(argv[3]);
	data->time_sleep = ft_atoui(argv[4]);
	if (narg == 6)
	{
		data->ends = 1;
		data->n_eats = ft_atoui(argv[5]);
	}
	if (data->n_philos == 0)
		ft_error("There is no philosopher.");
	if (data->n_philos > 200)
		ft_error("The maximum number of philosophers is 200.");
}

void parse_philo(t_data *data, t_philos *philo, unsigned int i)
{
	while (++i < data->n_philos)
	{
		philo[i].id = i + 1;
		philo[i].n_meals = 0;
		philo[i].last_meal = timer();
		philo[i].data = data;
		if (pthread_mutex_init(&philo[i].fork_r, NULL))
			ft_error("Error creating mutex.");
		if (pthread_mutex_init(&philo[i].mutex_eat, NULL))
			ft_error("Error creating mutex.");
		if (i != 0)
			philo[i].fork_l = &philo[i - 1].fork_r;
	}
	if (pthread_mutex_init(&philo->data->talk, NULL))
		ft_error("Error creating mutex.");
	if (pthread_mutex_init(&philo->data->stop_mutex, NULL))
		ft_error("Error creating mutex.");
	philo[0].fork_l = &philo[i - 1].fork_r;
}

void	parse(int narg, char **argv, t_data *data, t_philos *philo)
{
	printf("%s", (char *)&(HEADER));
	check_args(narg, argv);
	parse_data(narg, argv, data);
	parse_philo(data, philo, -1);
}
